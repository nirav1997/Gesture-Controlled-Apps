...
y_train = encode_one_hot(y_train, len(classes)+1)
y_test = encode_one_hot(y_test, len(classes)+1)
...
def initialize_params():
    ...
    W2 = tf.get_variable("W2", [3,3,6,8], initializer=tf.contrib.layers.xavier_initializer(seed=0))
    W3 = tf.get_variable("W3", [3,3,8,16], initializer=tf.contrib.layers.xavier_initializer(seed=0))
...
def model(X_train = X_train, y_tarin = y_train, X_test = X_test, y_test = y_test, lr=0.0009, num_epoches=100, mini_batch_size=64,print_cost=True):
    ...
    with tf.Session() as sess:
        ...
        predict_op = tf.arg_max(y_pred, 1, name="predict_op")
